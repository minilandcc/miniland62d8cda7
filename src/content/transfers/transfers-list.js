  ...
  useEffect( () => {
    if (asset){
      ...
      const fetchData = async() => {
        ...
        const resFunds = await TransfersFundListUser({data: {
          user: asset.item, index: index, items: items, 
          filters:{ name: props.search }
        }})
        console.log (resFunds)

        const resAssets = await TransfersAssetListUser({data: {
          user: asset.item, index: index, items: items, 
          filters:{ name: props.search }
        }})
        ...
        if (resFunds.stat && resAssets.stat) {
          setData(result)

          const resTotal = resFunds.data.count+resAssets.data.count
          setTotal(resTotal)

          setText(`${((index-1) * items +1)} - ${index * items < resTotal ? index * items : resTotal} of ${resTotal}`)
        }
      ...
      <div key={i}>
        <div className="d-flex px-3 mt-3">
          ...
          <div className="ms-auto text-end">         
              ...
              <span className="text-bold">{((item?.amount?.number || 0)/1000000).toFixed(0)}</span>
